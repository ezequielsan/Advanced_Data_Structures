...
size_t HashTable<K,V>::get_next_prime(size_t x) {
    if(x <= 2) {
        std::cout << "Tamanho da tabela: " << x << std::endl;
        return 3;
    }
    if(x % 2 == 0) x = x + 1;
    while(true) {
...
template <typename K, typename V>
HashTable<K,V>::HashTable(size_t tableSize, float lf, float mlf, 
    size_t (*h)(const K& k)) 
{
...
void HashTable<K,V>::rehash(size_t m) {
    ...
    HashTable<K, V> newHashTable(newSize, _load_factor, _max_load_factor, 
        _hashing);
    
    for (int i = 0; i < _table_size; i++) {
        for (auto it = (*_table)[i].begin(); it != (*_table)[i].end(); it++) {
            newHashTable.add((*it).first, (*it).second);
        }
    }

    delete _table;
    _table = newHashTable._table;
    newHashTable._table = nullptr;
    _table_size = newSize;
}
...
bool HashTable<K,V>::remove(const K& k) {
    size_t bucket = hash_code(k);
    for (auto it = (*_table)[bucket].begin(); it != (*_table)[bucket].end(); it++) {
        if ((*it).first == k) {
            (*_table)[bucket].erase(it);
            return true;
        }
    }
    return false;
...
V& HashTable<K,V>::operator[](const K& k) {
    ...
    (*_table)[bucket].push_back(std::make_pair(k, V()));
    return  (*_table)[bucket].back().second;
}
